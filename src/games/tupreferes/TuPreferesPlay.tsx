...
import { dilemmasForLevels } from './dilemmas';
...
export function TuPreferesPlayComponent({ players, config, onFinish, onQuit }: MiniGamePlayProps) {
  ...
  const [game, setGame] = useState<TuPreferesState>(() =>
    createTuPreferesState({ config: cfg, players, pool: dilemmasForLevels(getActiveCancelLevels()), seed: randomSeed() }),
  );
    ...
    void (async () => {
      try {
        ...
        setGame((cur) => {
          ...
          return createTuPreferesState({ config: cfg, players, pool: dilemmasForLevels(getActiveCancelLevels()), seed: randomSeed(), seen });
        });
      } catch {
