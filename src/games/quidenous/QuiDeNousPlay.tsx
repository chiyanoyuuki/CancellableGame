...
export function QuiDeNousPlayComponent({ players, config, onFinish, onQuit }: MiniGamePlayProps) {
  ...
  const [game, setGame] = useState<QuiDeNousState>(() =>
    createQuiDeNousState({ config: cfg, players, pool: promptsForLevels(getActiveCancelLevels()), seed: randomSeed() }),
  );
    ...
    void (async () => {
      try {
        ...
        setGame((cur) => {
          ...
          return createQuiDeNousState({ config: cfg, players, pool: promptsForLevels(getActiveCancelLevels()), seed: randomSeed(), seen });
        });
      } catch {
